...
describe("Testes da Classe ContaStandard", () => {
  test("verificar se instancia ContaStandard é feita corretamente", () => {
  ...
  test("instanciar conta Standard com valores validos", () => {
    const conta = new ContaStandard("1234", "12345", 1000, 4000);
  ...
  test("retorna mensagem de sucesso ao criar conta Standard com dados válidos e renda compatível", () => {
    const conta = new ContaStandard();
    expect(conta.criarConta("1234", "12345", 1000, 4000)).toBe("Conta Standard criada com sucesso");
    expect(conta.getAgencia()).toBe("1234");
    expect(conta.getConta()).toBe("12345");
    expect(conta.getSaldo()).toBe(1000);
  ...
  test("retorna erro ao criar conta com dados válidos e renda incompatível com a categoria Standard", () => {
    const conta = new ContaStandard();
    expect(() => conta.criarConta("1234", "12345", 1000, 5000)).toThrow("Renda incompatível com a categoria Standard");
  ...
  test("retorna erro ao criar conta com dados inválidos", () => {
    const conta = new ContaStandard();
    expect(() => conta.criarConta("123454", "123", 1000, 4000)).toThrow("Dados inválidos para cadastro");
  ...
  test("retorna mensagem de erro ao depositar -100 reais na conta Standard", () => {
    const conta = new ContaStandard();
    conta.criarConta("1234", "12345", 1000, 4000);
  ...
  test("retorna mensagem de erro ao sacar -100 reais da conta Standard", () => {
    const conta = new ContaStandard();
    conta.criarConta("1234", "12345", 1000, 4000);
  ...
  test("retorna erro ao fazer uma transferencia com valor acima do limite diário", ()=>{
    ...
    contaEmissor.criarConta("0001", "12345", 2500, 4000)
    contaReceptor.criarConta("0001", "78945", 500)

    expect(() => contaEmissor.transferir(2000, "0001", "78945").toThrow("Valor de transferência acima do limite diário"))
  ...
  test("retorna erro ao fazer um pix com valor acima do limite diário", () => {
    ...
    expect(() => contaEmissor.pix(2000, "email@email.com", "email")).toThrow("Valor de transferência acima do limite diário")
    expect(contaEmissor.getSaldo()).toBe(2500)
    expect(contaReceptor.getSaldo()).toBe(500)
